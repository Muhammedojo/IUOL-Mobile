...
//     try {
//       Response response =
//           await doPostRequest(verifyScratchCardPinEndpoint, body);
//       statusCode = response.statusCode;
...
import 'package:ioul/model/login.dart';
...
class ApiProvider {
    ...
    try {
      ...
      return switch (response.statusCode) {
        201 || 200 => right(ApiResponseImpl(
            ...
            isList: null,
            fromJson: (e) => LoginData.fromJson(e['data']),
          )),
        _ => left(
            ValidationFailure(
  ...
  Future<Either<Failure, ApiResponseImpl<String>>> verifyScratchCardPin(
      {required String pin}) async {
    try {
      final response = await dioClient.request(
        type: MethodType.post,
        url: verifyScratchCardPinEndpoint,
        body: {
          "pin": pin,
        },
      );
  ...
  Future<Either<Failure, ApiResponseImpl<String>>> makePayment(
      {required MakePaymentData data}) async {
    log("payment type: ${data.applicationData?.paymentType}");
    try {
      final response = await dioClient.request(
        type: MethodType.post,
        url: payment,
        body: {
          "amount": data.applicationData?.amount,
          ...
          "payment_type": data.applicationData?.paymentType?.capitalWords(),
        },
      );
      log(response.data);
      return switch (response.statusCode) {
    ...
    try {
      ...
      return switch (response.statusCode) {
        201 || 200 => right(ApiResponseImpl(
            myData: jsonDecode(response.data),
            isList: null,
            fromJson: (e) => ApplicationFormData.fromJson(e['data']),
          )),
        _ => left(
            ValidationFailure(
              errorMessageHandler(jsonDecode(response.data)['data']),
